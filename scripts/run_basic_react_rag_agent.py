...
USAGE:
    ...
    # Single query mode
    uv run python scripts/run_basic_react_rag_agent.py --query "What does Kyndryl cover for surgeries?"
"""
import sys
...
SYSTEM_MESSAGE = SystemMessage(content="""You are a helpful AI assistant with access to Kyndryl's internal documentation.

IMPORTANT INSTRUCTIONS:
1. When users ask about Kyndryl policies, benefits, procedures, or documentation:
   - Use the search_kyndryl_documents tool to find relevant information
...
2. For general questions that don't require Kyndryl-specific information:
   - Answer directly from your knowledge
   - Be clear about what is general knowledge vs. Kyndryl-specific

3. Best practices:
   - Search with specific, focused queries
   - If the first search doesn't yield good results, try rephrasing
   - Summarize findings clearly and concisely
   - Always provide source citations for Kyndryl-specific information

Example interactions:
- User: "What does Kyndryl cover for surgeries?"
  → Use tool: search_kyndryl_documents("Kyndryl surgery coverage benefits")
...
EXAMPLE_QUERIES = [
    {
        "thread_id": "example-1",
        "query": "How much does Kyndryl cover for surgeries?"
    },
    {
        "thread_id": "example-2", 
        "query": "What are the hospitals covered by Kyndryl's health insurance?"
    },
    {
        "thread_id": "example-3",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Kyndryl RAG Agent - Search internal documentation",
        ...
        epilog=""" Examples:
            ...
            # Single query
            uv run python scripts/run_basic_react_rag_agent.py --query "What does Kyndryl cover?"
    ...
    try:
        ...
        # Interactive mode
            ...
            run_interactive_chat(
                agent=agent,
                thread_id=args.thread_id,
                title="KYNDRYL RAG AGENT - Interactive Mode"
            )
